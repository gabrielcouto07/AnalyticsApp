...
def parse_custos_file(file_bytes: bytes, filename: str) -> Dict[str, Any]:
    ...
    # Find NFs sheet
    nfs_sheet = next((s for s in wb.sheetnames if "NF" in s.upper() and "ENTRADA" in s.upper()), None)
    df_nfs = pd.DataFrame()
    meta = {}
    if nfs_sheet:
        ws_nfs = wb[nfs_sheet]
    ...
    # Find Consolidado sheet
    cons_sheet = next((s for s in wb.sheetnames if "CONSOLIDADO" in s.upper() and "RESUMO" not in s.upper()), None)
